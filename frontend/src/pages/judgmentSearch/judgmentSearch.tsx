      ...
      <Container>
        ...
        <Hero>
          <HeroInner>
            ...
            <HeroSubtitle>
              Review your courtroom performances and track your progress.
            </HeroSubtitle>
            ...
            <HeroStats>
              ...
              <HeroDivider />
              <Stat>
                <StatIcon><Award size={15} strokeWidth={1.8} /></StatIcon>
                <StatNumber>{earnedCount}/{BADGES.length}</StatNumber>
        ...
        <FilterBar>
          <FilterBarInner>
            <SearchWrap>
              <SearchIconWrap>
                <Search size={14} strokeWidth={2.2} color="#94a3b8" />
              </SearchIconWrap>
              <SearchInput
  ...
  setSearch(e.target.value)
        ...
        <Main>
              ...
              <AnimatePresence mode="popLayout">
                {loading ? (
                  ...
                  >
                    <EmptyState>
                      <EmptyIconWrap>
                        <FolderOpen size={40} strokeWidth={1.3} color="#93c5fd" />
                      </EmptyIconWrap>
                      <EmptyTitle>No records found</EmptyTitle>
                      <EmptyMsg>Try adjusting your filters or search query.</EmptyMsg>
                      <EmptyBtn onClick={clearAll}>
                    ...
                    return (
                      ...
                      >
                        <Row onClick={() => toggle(game.id)} $open={isOpen}>
                          <RowLeft>
                            <RankBadge style={{ background: color + "18", color }}>
                              #{idx + 1}
                            </RankBadge>
                            <CaseBlock>
                              <CaseTitle>{game.case}</CaseTitle>
                              <CaseMeta>
                          ...
                          <RowRight>
                            <ScoreBlock>
                              <Score style={{ color }}>{game.score}%</Score>
                              ...
                              <ExpandedPanel>
                                <PillRow>
                                  ...
                                  <Pill>
                                    ...
                                    <PillValue>{game.duration}</PillValue>
                                  </Pill>
                                </PillRow>
                                ...
                                <BarSection>
                                  <BarRow>
                                    <BarMeta>
                                      <BarLabelWrap>
                                        <TrendingUp size={12} strokeWidth={2} color="#64748b" />
                                        <BarLabel>Score</BarLabel>
                                      </BarLabelWrap>
                                      <BarValue style={{ color: scoreColor(game.score) }}>
                                        {game.score}%
                                      </BarValue>
                                    </BarMeta>
                                    <BarTrack>
                                      <BarFill style={{ width: `${game.score}%`, background: scoreColor(game.score) }} />
                                    </BarTrack>
                                  </BarRow>
                                  ...
                                  <BarRow>
                                    <BarMeta>
                                      <BarLabelWrap>
                                        <Target size={12} strokeWidth={2} color="#64748b" />
                                        <BarLabel>Accuracy</BarLabel>
                                      </BarLabelWrap>
                                      <BarValue style={{ color: scoreColor(game.accuracy) }}>
                                        {game.accuracy}%
                                      </BarValue>
                                    </BarMeta>
                                    <BarTrack>
                                      <BarFill style={{ width: `${game.accuracy}%`, background: scoreColor(game.accuracy) }} />
                                    </BarTrack>
                                  </BarRow>
                                </BarSection>
                              </ExpandedPanel>
          ...
          <Right>
            ...
            <SummaryCard>
              {[
                { val: "980", label: "Total Points", Icon: Trophy   },
                { val: "22",  label: "Cases Played", Icon: BookOpen },
                { val: "72%", label: "Win Rate",     Icon: Swords   },
              ].map(({ val, label, Icon }) => (
              ...
              <ProgressWrap>
                <ProgressMeta>
                  ...
                  <ProgressVal>980 / 1150</ProgressVal>
                </ProgressMeta>
                <ProgressTrack>
                  <ProgressFill style={{ width: `${(980 / 1150) * 100}%` }} />
                </ProgressTrack>
              </ProgressWrap>
...
>
  ...
  Challenge Top Players
            ...
            <SectionTitle style={{ margin: "24px 0 10px" }}>
            ...
            <BadgeCard>
              <BadgeGrid>
                {BADGES.map((b) => (
                  <BadgeItem key={b.label} $earned={b.earned} title={b.desc}>
                    <BadgeIconWrap $earned={b.earned}>
                      <b.Icon size={20} strokeWidth={1.8} />
                    </BadgeIconWrap>
                    <BadgeName $earned={b.earned}>{b.label}</BadgeName>
...
const FilterBar = styled.div`
  ...
  box-shadow: 0 2px 14px rgba(20,50,160,0.07);
  position: sticky;
  top: 0;
  z-index: 20;
`;
...
const FilterChip = styled.button<{ $active: boolean }>`
  padding: 5px 13px;
  border-radius: 20px;
  border: 1.5px solid ${p => p.$active ? '#1e3a8a' : '#dbeafe'};
  background: ${p => p.$active ? 'linear-gradient(135deg,#1e3a8a,#2563eb)' : '#f8faff'};
  ...
  &:hover { border-color: #2563eb; color: ${p => p.$active ? 'white' : '#1e3a8a'}; }
`;
...
const Card = styled.div`
  ...
  border-radius: 16px;
  overflow: hidden;
  box-shadow: 0 2px 24px rgba(20,50,160,0.09);
...
const Row = styled.div<{ $open: boolean }>`
  ...
  align-items: center;
  cursor: pointer;
  background: ${p => p.$open ? '#f8faff' : 'white'};
...
const CaseMeta = styled.div`
  ...
  flex-wrap: wrap;
`;
const ModePill = styled.span<{ $mode: string }>`
  display: inline-flex;
  align-items: center;
  gap: 4px;
  background: ${p => p.$mode === 'AI' ? '#eff6ff' : '#f0fdf4'};
  color: ${p => p.$mode === 'AI' ? '#1d4ed8' : '#15803d'};
  border: 1px solid ${p => p.$mode === 'AI' ? '#bfdbfe' : '#bbf7d0'};
  border-radius: 5px;
  padding: 2px 8px;
  font-size: 11px;
  font-weight: 700;
...
const BarFill = styled.div`
  height: 100%;
  border-radius: 99px;
  animation: ${fillAnim} 0.7s ease both;
`;
...
const SummaryIconWrap = styled.div`
  width: 32px;
  height: 32px;
  background: #eff6ff;
  border-radius: 8px;
...
const ProgressFill  = styled.div`
  ...
  animation: ${fillAnim} 0.9s ease both;
`;
...
const BadgeItem = styled.div<{ $earned: boolean }>`
  background: ${p => p.$earned ? '#f0f6ff' : '#f8fafc'};
  border: 1.5px solid ${p => p.$earned ? '#bfdbfe' : '#e2e8f0'};
  border-radius: 12px;
  padding: 14px 10px;
  text-align: center;
  ...
  cursor: default;
  &:hover {
    transform: ${p => p.$earned ? 'translateY(-3px)' : 'none'};
    box-shadow: ${p => p.$earned ? '0 4px 16px rgba(37,99,235,0.14)' : 'none'};
  }
`;
const BadgeIconWrap = styled.div<{ $earned: boolean }>`
  display: flex;
  align-items: center;
  justify-content: center;
  width: 40px;
  ...
  border-radius: 10px;
  margin: 0 auto 8px;
  background: ${p => p.$earned ? 'linear-gradient(135deg,#dbeafe,#eff6ff)' : '#f1f5f9'};
  color: ${p => p.$earned ? '#1e3a8a' : '#94a3b8'};
  filter: ${p => p.$earned ? 'none' : 'grayscale(1)'};
`;
const BadgeName = styled.div<{ $earned: boolean }>`
  font-size: 11px;
  font-weight: 700;
  color: ${p => p.$earned ? '#1e3a8a' : '#94a3b8'};
  font-family: sans-serif;
